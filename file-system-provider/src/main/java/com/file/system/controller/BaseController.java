...
import com.file.system.common.*;
import com.file.system.common.CommitAction.CommitType;
import com.file.system.entity.*;
import com.file.system.service.impl.ReposServiceImpl;
import com.file.system.service.impl.UserServiceImpl;
import com.file.system.utils.DateFormat;
import com.file.system.utils.Encrypt.MD5;
import com.file.system.utils.GitUtil.GITUtil;
import com.file.system.utils.LuceneUtil.LuceneUtil2;
import com.file.system.utils.ReadProperties;
import com.file.system.utils.ReturnAjax;
import com.file.system.utils.SvnUtil.SVNUtil;
...
//			}
	...
	protected DocChange.DocChangeType getDocChangeType_FSM(Repos repos, Doc doc, Doc dbDoc, Doc localEntry, Doc remoteEntry)
	{						
		//dbDoc不存在
		if(dbDoc == null)
		{
			...
			if(localEntry != null && localEntry.getType() != 0)
			{
				...
				return DocChange.DocChangeType.LOCALADD;
			}
			...
			if(remoteEntry != null && remoteEntry.getType() != 0)
			{
				...
				return DocChange.DocChangeType.REMOTEADD;
			}
			
			//未变更
			//System.out.println("getDocChangeType_FSM() 未变更(dbDoc不存在/localDoc不存在/remoteDoc不存在):" + doc.getDocId() + " " + doc.getPath() + doc.getName());
			return DocChange.DocChangeType.NOCHANGE;
		}
		
		//dbDoc存在
		if(localEntry == null || localEntry.getType() == 0)
		{
			DocChange.DocChangeType remoteChangeType = getRemoteChangeType(repos, dbDoc, remoteEntry);

			if(remoteChangeType == DocChange.DocChangeType.NOCHANGE || remoteChangeType == DocChange.DocChangeType.REMOTEDELETE)
			{
				//本地文件/目录删除
				System.out.println("getDocChangeType_FSM() 本地删除:" + doc.getDocId() + " " + doc.getPath() + doc.getName() + " dbDoc存在 localEntry不存在");
				return DocChange.DocChangeType.LOCALDELETE;
			}
		...
		if(localEntry.getType() == 1)
		{
			if(dbDoc.getType() == 2)
			{
				...
				return DocChange.DocChangeType.LOCALDIRTOFILE;
			}
			...
			if(isDocLocalChanged(repos, dbDoc, localEntry))
			{
				...
				return DocChange.DocChangeType.LOCALCHANGE;
			}
			...
			if(remoteEntry == null || remoteEntry.getType() == 0)
			{
				...
				return DocChange.DocChangeType.REMOTEDELETE;
			}
			...
			if(remoteEntry.getType() == 2)
			{
				...
				return DocChange.DocChangeType.REMOTEFILETODIR;
			}
			...
			if(isDocRemoteChanged(repos, dbDoc, remoteEntry))
			{
				...
				return DocChange.DocChangeType.REMOTECHANGE;
			}
		...
		if(localEntry.getType() == 2)
		{
			if(dbDoc.getType() == 1)
			{
				...
				return DocChange.DocChangeType.LOCALFILETODIR;
			}
			...
			if(remoteEntry.getType() == 1)
			{
				...
				return DocChange.DocChangeType.REMOTEDIRTOFILE;
			}
		...
		return DocChange.DocChangeType.UNDEFINED;
	}
...
//		}	
	...
	private DocChange.DocChangeType getRemoteChangeType(Repos repos, Doc dbDoc, Doc remoteEntry)
	{
