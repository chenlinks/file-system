...
import com.file.system.utils.Encrypt.Base64File;
import com.file.system.utils.Encrypt.MD5;
import com.file.system.utils.FileUtil.CompressPic;
...
public class BaseFunction{
	...
	protected void insertAddFileAction(List<CommitAction> actionList, Doc doc, boolean isSubAction) {
    	...
    	action.setAction(CommitAction.CommitType.ADD);
    	action.setDoc(doc);
	...
	protected void insertDeleteAction(List<CommitAction> actionList, Doc doc) {
    	...
    	CommitAction action = new CommitAction();
    	action.setAction(CommitAction.CommitType.DELETE);
    	action.setDoc(doc);
    	actionList.add(action);
	}
	...
	protected void insertModifyAction(List<CommitAction> actionList, Doc doc) {
    	...
    	CommitAction action = new CommitAction();
    	action.setAction(CommitAction.CommitType.MODIFY);
    	action.setDoc(doc);
    	actionList.add(action);	
	}
	
	protected void insertAction(List<CommitAction> actionList, Doc doc, CommitAction.CommitType actionType) {
		printObject("insertModifyAction:", doc);
    	CommitAction action = new CommitAction();
    	action.setAction(actionType);
    	action.setDoc(doc);
	...
	public static String getFileEncode(String filePath)
	{
        ...
        try {
			 charsetName = FileTypeUtil.getTypeByPath(filePath);
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
        }
